...
shared_ptr<PhysicsObject> makePhysicsObject(Prefab& definition)
{
    ...
    if (definition.name == "Super Jump" || definition.name == "Lightning Speed") return make_shared<PowerUp>(vec3(0, 0, 0), quat(1, 0, 0, 0), definition.model, 1.0f, definition.name);
